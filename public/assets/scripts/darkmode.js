function toggleClass(element, className) {
	// better support for old old browsers
...
function toggleDarkMode() {
	...
	const usingLightMode =
		document.documentElement.className.indexOf("light-mode") > -1;
	document
		.querySelector("button.dark-mode")
		.setAttribute("data-aftercontent", usingLightMode ? ": off" : ": on");
	if (document.getElementById("darkmodeinput"))
...
function toggleReducedMotion() {
	...
	const usingRM =
		document.documentElement.className.indexOf("reduced-motion") > -1;
	document
		.querySelector("button.reduced-motion")
		.setAttribute("data-aftercontent", usingRM ? ": on" : ": off");
	if (document.getElementById("reducedmotioninput"))
...
function toggleTheme(theme) {
	...
	if (currentClasses.indexOf(" theme-") > -1) {
		var themeIndexString = currentClasses.substring(
			currentClasses.indexOf(" theme-") + 1,
		);
		var themeString = themeIndexString.substring(
			0,
			themeIndexString.indexOf(" "),
		);
	...
	document
		...
		.setAttribute("data-aftercontent", ": " + themeDisplayName);
...
function updateSiteLogo() {
	...
	if (currentClasses.indexOf(" theme-") > 1) {
		...
		var theme = themeIndexString
			.substring(0, themeIndexString.indexOf(" "))
			.replace("theme-", "");

		if (
			theme == "mintchoco" ||
			(theme == "unbeatable" && currentClasses.indexOf(" light-mode") < 0)
		) {
...
function freezeGifs() {
	...
	for (var i = 0; i < x.length; i++) {
		...
		x[i].onmouseover = function () {
			if (
				this.src.endsWith("-static.png") ||
				this.src.includes("-static.png?")
			) {
