// modified from @lacymorrow/album-art, licensed under MIT
"use strict";
...
((root, cx) => {
	...
	const albumArt = async (artist, options, cb) => {
		...
		if (opts.track !== null) {
			method = "track";
			fallbackQuery = `${primaryArtist} ${opts.track}`;
			query = `artist:${primaryArtist} track:${opts.track}`;
		} else if (opts.album !== null) {
			method = "album";
		...
		const extractImage = (items) => {
			...
			if (method === "track") {
				// Track results: match against item.name directly
				match =
					items.find((item) =>
						item.artists.includes(primaryArtist.toLowerCase()),
					) || items[0].album;
			} else if (method === "artist") {
				// Artist results: match against item.name directly
				match =
					items.find(
						(item) => item.name.toLowerCase() === primaryArtist.toLowerCase(),
					) || items[0];
			} else {
				...
				match =
					items.find((item) =>
						item.artists.some(
							(a) => a.name.toLowerCase() === primaryArtist.toLowerCase(),
						),
					) || items[0];
			}
			...
			if (opts.size === SIZES.SMALL) {
				return [smallest.url, match.name];
			}

			if (opts.size === SIZES.MEDIUM && images.length > 1) {
				return [images[1].url, match.name];
			}

			return [largest.url, match.name];
		};
