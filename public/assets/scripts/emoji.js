...
const modifierCSS =
	".emojiText::after { display: inline; } .emojiText.hoof::after { content: '􁙒􁘨'; } .emojiText.paw::after { content: '􁙐􁘈'; }";

function parseEmoji(text) {
	var r = String.raw;
	var regexPartial = r`(\p{Emoji}|[\uE000-\uF8FF\u{F0000}-\u{FFFFD}\u{100000}-\u{10FFFD}])(?:\p{EMod}|[\u{E0020}-\u{E007E}]+\u{E007F}|\uFE0F?\u20E3?)`;
	var regex = RegExp(
		r`\p{RI}{2}|(?![#*\d](?!\uFE0F?\u20E3))${regexPartial}(?:\u200D${regexPartial})*`,
		"gu",
	);
	...
	return text.replaceAll("↩", "↩️").replaceAll(regex, function (emoji) {
		...
		var title =
			code in privateMSAlt ? ' aria-label="' + privateMSAlt[code] + '"' : "";
		if (hoofEmojis.indexOf(code) !== -1) {
			classN += " hoof";
		}
		if (pawEmojis.indexOf(code) !== -1) {
			classN += " paw";
		}
